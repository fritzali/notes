...
class TrajectoryHistory:
    """Dynamically sized storage for particle trajectory data.
    ...
    "store_dt" threshold can be used to limit storage to a subsampled
    sequence, matching the original sampling interval pattern.

    Parameters
    ----------
    store_dt : float or None, minimum elapsed simulation time between stored snapshots, None → store every call to :meth:`record`
    """
    ...
    def record(self, state, force: bool = False):
        ...
        Parameters
        ----------
        state : ParticleState, current simulation state.
        force : bool, if True, bypass the "store_dt" gate (used for the initial point).
        """
        if self._finalized:
    ...
    def kinetic_energy(self, m: np.ndarray, relativistic: bool = True) -> np.ndarray:
        ...
        Parameters
        ----------
        m : ndarray, shape (N,), particle masses [kg]
        relativistic : bool, whether to use the relativistic formula K=(γ-1)mc²

        Returns
        -------
        K : ndarray, shape (S, N), kinetic energy of each particle at each stored time step [J].
        """
...
def relative_energy_error(
    ...
    Returns
    -------
    err : ndarray, shape (S, N), "(K(t) - K(0)) / K(0)"
    """
...
def check_dt_resolution(
    ...
    q, m : float, particle charge and mass.
    B_mag : float, representative magnetic field magnitude [T].
    warn_threshold : float, emit a warning if dt / T_c > warn_threshold.
...
def suggest_dt(
    ...
    Parameters
    ----------
    steps_per_gyration : float, target number of integration steps per gyro period.
