...
picsim — generate stellar catalogues


varsim — generate noiseless stellar/planetary light curves


payload — generate instrumental systematics


platonium — run the PlatoSim multi-camera, multi-quarter simulation
...
where L∗ is stellar luminosity, S is incident stellar flux in Earth units, and T_{\mathrm{eq}} is the planetary equilibrium temperature. The student may also discuss the simplified zero-albedo form if that is the version emphasized in the course.
