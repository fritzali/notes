...
plt.savefig('build/theo_bremsstrahlung.pdf')
...
def ncc(r, rc = 150.0, rhog = 6.5e4, b = 0.65):
	return rhog * (1 + (r / rc)**2)**(-1.5 * b)

def cc(r, rc1 = 25.0, rhog1 = 6.5e5, b1 = 0.55, rc2 = 200.0, rhog2 = 4.0e4, b2 = 0.65):
	inner = rhog1 * (1 + (r / rc1)**2)**(-1.5 * b1)
	outer = rhog2 * (1 + (r / rc2)**2)**(-1.5 * b2)
	return (inner**2 + outer**2)**0.5

r = np.logspace(1.0, 3.3, 333)

plt.plot(r, nfw(r), label=r'Total Density', ls='-', c='k', lw=0.8)
plt.plot(r, cc(r), label=r'Cooling Core', ls=(0, (8, 2)), c='k', lw=0.8)
plt.plot(r, ncc(r), label=r'Noncool Core', ls=(0, (3, 1)), c='k', lw=0.8)
...
plt.savefig('build/theo_halo.pdf')
...
def cc_dens(r, rc1 = 30, b1 = 0.5, n1 = 5e-2, rc2 = 200, b2 = 0.7, n2 = 1e-3):
	return (n1**2 * (1 + (r / rc1)**2)**(- 3 * b1) + n2**2 * (1 + (r / rc2)**2)**(- 3 * b2))**(1/2)
...
plt.savefig('build/theo_profiles.pdf')

plt.close()

plt.plot(r, cc_dens(r), label='Cooling Core', ls='-', c='k', lw=0.8)
plt.plot(r, ncc_dens(r), label='Noncool Core', ls=(0, (8, 2)), c='k', lw=0.8)
...
plt.savefig('build/theo_density.pdf')

plt.close()

plt.plot(r, cc_pres(r), label='Cooling Core', ls='-', c='k', lw=0.8)
plt.plot(r, ncc_pres(r), label='Noncool Core', ls=(0, (8, 2)), c='k', lw=0.8)
...
plt.savefig('build/theo_pressure.pdf')

plt.close()

plt.plot(r, cc_temp(r), label='Cooling Core', ls='-', c='k', lw=0.8)
plt.plot(r, ncc_temp(r), label='Noncool Core', ls=(0, (8, 2)), c='k', lw=0.8)

plt.xlabel(r'$r \mathbin{/} \unit{\kilo\parsec}$')
plt.ylabel(r'$kT \mathbin{/} \unit{\kilo\electronvolt}$')

plt.xscale('log')

plt.legend()

plt.savefig('build/theo_temperature.pdf')

plt.close()

plt.plot(r, cc_entr(r), label='Cooling Core', ls='-', c='k', lw=0.8)
plt.plot(r, ncc_entr(r), label='Noncool Core', ls=(0, (8, 2)), c='k', lw=0.8)
...
plt.savefig('build/theo_entropy.pdf')
...
plt.savefig('build/chan_spectra.pdf')
...
plt.savefig('build/chan_density.pdf')
...
plt.savefig('build/chan_temperature.pdf')
...
plt.savefig('build/chan_entropy.pdf')
...
plt.savefig('build/chan_pressure.pdf')
...
plt.savefig('build/chan_profiles.pdf')
...
plt.savefig('build/xmmn_abundance.pdf')
...
plt.savefig('build/xmmn_density.pdf')
...
plt.savefig('build/xmmn_entropy.pdf')
...
plt.savefig('build/xmmn_fgas.pdf')
...
plt.savefig('build/xmmn_pressure.pdf')
...
plt.savefig('build/xmmn_temperature.pdf')
...
plt.savefig('build/xmmn_profiles.pdf')
