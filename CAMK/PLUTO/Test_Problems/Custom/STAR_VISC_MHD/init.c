...
//ccm040726--here set the rho(x1[i]) <= rho everywhere
  if (x1[i] > x1[IBEG+1] && d->Vc[RHO][k][j][i]< g_inputParam[DFLOOR]*1.e-3){
//ccm--save values of rho and cs2 before reset to dfloor
     dden=d->Vc[RHO][k][j][i];
     cs2=g_gamma*d->Vc[PRS][k][j][i]/d->Vc[RHO][k][j][i];
     d->Vc[RHO][k][j][i]=g_inputParam[DFLOOR]*1.e-3;     
...
//  if(x2[j]<0.5*CONST_PI-atan(1.25*g_inputParam[EPS]) && d->Vc[RHO][k][j][i]<
  if(x2[j]<0.5*CONST_PI-atan(3.*g_inputParam[EPS]) && d->Vc[RHO][k][j][i]<
          g_inputParam[DFLOOR]*1.e-3)d->Vc[TRC][k][j][i]=0.0;
...
//  if(x2[j]<0.5*CONST_PI-atan(1.25*g_inputParam[EPS]) && d->Vc[RHO][k][j][i]<
  if(x2[j]<0.5*CONST_PI-atan(3.*g_inputParam[EPS]) && d->Vc[RHO][k][j][i]<
          g_inputParam[DFLOOR])d->Vc[TRC][k][j][i]=0.0;
//ccm--for full [0,PI] half-plane: 
  if(x2[j]>0.5*CONST_PI+atan(3.*g_inputParam[EPS]) && d->Vc[RHO][k][j][i]<
          g_inputParam[DFLOOR])d->Vc[TRC][k][j][i]=0.0;
// Ex3e[0][j][IBEG-1] = 0.0;
 ...
 *(Bgi[1]+d->Vc[BX2][k][j][i]);
  ...
  if (x2[j]<0. || x2[j]>3.1415926) db2dth=2.*d->Vc[BX3][k][j][i+1]/(x2[j]-x2[j-1]);
...
//  dbdth=MINMOD_LIMITER(db1dth,db2dth);
  ...
  if (x2[j]<0. || x2[j]>3.1415926) dvar2dth=2.*d->Vc[VX3][k][j][i+1]/(x2[j]-x2[j-1]);
 // if (x2[j]>1.5708) dvar1dth=0.;
...
//ccm290515--added fabs to cos for the [0,PI] case!
  ...
  *(dvardth+d->Vc[VX3][k][j][i+1]*fabs(cos(x2[j]))/sin(x2[j]))
  +(d->Vc[BX1][k][j][i+1]+Bg[0])/r[i+1]*d->Vc[BX3][k][j][i+1]
  +(d->Vc[BX2][k][j][i+1]+Bg[1])/r[i+1]*(dbdth+fabs(cos(x2[j]))/sin(x2[j])
 ...
 *x1[IBEG]*sin(x2[j])) {
  ...
  *(dvardth+d->Vc[VX3][k][j][i+1]*fabs(cos(x2[j]))/sin(x2[j]))
  ...
  +(d->Vc[BX2][k][j][i+1])/r[i+1]*(dbdth+fabs(cos(x2[j]))/sin(x2[j])
...
//ccm--for [0,pi/2] case only the line below is enough:
...
//ccm--line below added for full [0,PI] half-plane
    ...
    d->Vc[PRS][k][j][i] = eps2*pow(coeff,g_gamma/(g_gamma-1.));//eps2*pow(coeff,5./2.)
       ...
       d->Vc[VX1][k][j][i] = -g_inputParam[ALPHAV]/sin(x2[j])
       *eps2*(10.-32./3.*lambda*g_inputParam[ALPHAV]*g_inputParam[ALPHAV]
