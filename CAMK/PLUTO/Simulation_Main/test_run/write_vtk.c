...
/* --------------------------------------------------------
  ...
  if (vel_field || mag_field) { 
    DOM_LOOP(k,j,i){ 
//ccm--060521 changes for mag. field, to output total B0+B1, not
//-- only the varying part B1. 
if (vel_field){
      DIM_EXPAND(v[0] = V[0][k][j][i]; x1 = grid->x[IDIR][i]; ,
                 v[1] = V[1][k][j][i]; x2 = grid->x[JDIR][j]; ,
                 v[2] = V[2][k][j][i]; x3 = grid->x[KDIR][k];)
  }else{
 #if BACKGROUND_FIELD == YES  //csk
   BackgroundField (x1, x2, x3, Bg);
 #else
   Bg[0]=0;Bg[1]=0;Bg[2]=0;
 #endif

        DIM_EXPAND(v[0] = V[0][k][j][i]+Bg[0]; x1 = grid->x[IDIR][i]; ,
                 v[1] = V[1][k][j][i]+Bg[1]; x2 = grid->x[JDIR][j]; ,
                 v[2] = V[2][k][j][i]+Bg[2]; x3 = grid->x[KDIR][k];)}
//ccm--end of change
