...
/*!
  ...
  ======================================================================================
  ======================================================================================
  Modified as a local working directory copy, without touching the original PLUTO file:
  Added a single global assignment to expose the radial grid index to user defined
  opacities, whose signature does not otherwise carry any grid position information.
  ======================================================================================
  ======================================================================================
...
void RadImplicitNR(Rad_data * rad_data, double ** source, int ibeg, int iend){
	...
	for (i = ibeg; i <= iend; i++){
		u = ucons[i];
		v = uprim[i];
		g_i_rad = i;   /* Added to expose current radial index to user
                          defined opacities, which has no index in its
                          own fixed signature. Written here only,
                          declared and defined in initialization. */
