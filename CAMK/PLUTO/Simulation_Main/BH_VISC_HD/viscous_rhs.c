...
/* --------------------------------------------------------
  ...
  /* -- 1b. Build rhs in the X1-direction and update --
    ...
    for (i = beg; i <= end; i++){
      ...
      if (nu1 > nu_floor) {
        cool = cost*vc[TRC]*dt*0.5*(pow(ViF[i][iMR],2)+pow(ViF[i][iMTH],2)
  ...
  /* -- 2b. Build rhs in the X2-direction and update -- */
    ...
    for (j = beg; j <= end; j++){
      ...
      Visc_nu(vc, x1[i], x2[j], x3[k], &nu1, &nu2);
      if (nu1 > nu_floor) {
        cool = cost*vc[TRC]*dt*0.5*(pow(ViF[j][iMR],2)+pow(ViF[j][iMTH],2)
  ...
  /* -- 3b. Build rhs in the X3-direction -- */
    ...
    for (k = beg; k <= end; k++){
      ...
      Visc_nu(vc, x1[i], x2[j], x3[k], &nu1, &nu2);
      if (nu1 > nu_floor) {
        cool = cost*vc[TRC]*dt*0.5*(pow(ViF[k][iMR],2)+pow(ViF[k][iMTH],2)
